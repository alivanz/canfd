...
/*
 * sdc_mcp251xfd - SUNIX SDC SPI MCP251xFD CAN controller driver
 *
...
/* Change by SUNIX, distinguish with source tree driver */
#define DEVICE_NAME "sdc_mcp251xfd"

/* Add by SUNIX */
#define MCP251XFD_TERMINATION_EANBLED		120
#define MCP251XFD_TERMINATION_DISABLED		CAN_TERMINATION_DISABLED
const u16 termination_const[] = {MCP251XFD_TERMINATION_DISABLED, MCP251XFD_TERMINATION_EANBLED};
...
/* Add by SUNIX */
static int mcp251xfd_set_termination(struct net_device *ndev, u16 term_req)
{
...
static int mcp251xfd_chip_ecc_init(struct mcp251xfd_priv *priv)
{
	...
	/* Change by SUNIX */
	ram = kzalloc(max_size, GFP_KERNEL);
	if (!ram)
		return -ENOMEM;

	addr = MCP251XFD_RAM_START;
	for (i = 0; i < MCP251XFD_RAM_SIZE; i++) {
		err = regmap_raw_write(priv->map_reg, addr, ram, max_size);
		if (err)
			break;
		i += max_size;
		addr += max_size;
			   ...
			   const struct mcp251xfd_hw_tef_obj *hw_tef_obj)
{
	...
	/* Change by SUNIX */
		...
		can_rx_offload_get_echo_skb(&priv->offload,
					    ...
					    hw_tef_obj->ts);
#else
		can_get_echo_skb(priv->ndev, mcp251xfd_get_tef_tail(priv));
#endif
	stats->tx_packets++;
...
#elif KERNEL_VERSION(4, 14, 85) <= LINUX_VERSION_CODE
...
/* Add by SUNIX */
static irqreturn_t mcp251xfd_can_irq_handler(int irq, void *dev_id)
{
...
static int mcp251xfd_open(struct net_device *ndev)
{
	...
	/* Change by SUNIX */
	err = request_threaded_irq(spi->irq, mcp251xfd_can_irq_handler, mcp251xfd_irq,
				   IRQF_SHARED, dev_name(&spi->dev),
				   priv);
	if (err)
...
static const struct of_device_id mcp251xfd_of_match[] = {
	{
		.compatible = "microchip,sdc_mcp251xfd",
...
static int mcp251xfd_probe(struct spi_device *spi)
{
	...
	/* Add by SUNIX */
	if (pdata->can_termination) {
		priv->can.termination_const = termination_const;
		priv->can.termination_const_cnt = ARRAY_SIZE(termination_const);
		priv->can.termination = MCP251XFD_TERMINATION_EANBLED; /* Default enable by spi created */
		priv->can.do_set_termination = mcp251xfd_set_termination;
	}
...
MODULE_AUTHOR("Morris Ku <morris_ku@sunix.com>");
MODULE_DESCRIPTION("SUNIX SDC SPI Microchip MCP251xFD Family CAN driver");
